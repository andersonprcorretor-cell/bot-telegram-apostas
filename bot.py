...
def calcular_estatisticas_avancadas(home_id, away_id):
    ...
    over_25 = min(max(over_25, 55), 88)
    return over_15, over_25

def processar_jogos():
    # Busca os jogos do dia (ou troca para + timedelta(days=1) se preferir a grade de amanhã)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if fixtures:
                msg = "⚽ <b>MERCADO GLOBAL - ANÁLISE INTELIGENTE</b> ⚽\n"
                msg += f"📅 <b>Data:</b> {data_str}\n\n"
                
                contador = 0
                for item in fixtures:
                    # Aumentado para exibir até 8 oportunidades de várias ligas do mundo
                    if contador >= 8:
                        break
                    ...
                    msg += f"📈 <b>Projeções:</b> Over 1.5 (<code>{p15}%</code>) | Over 2.5 (<code>{p25}%</code>)\n"
            ...
            return "⚠️ <i>Nenhuma partida encontrada para os critérios atuais. Tente consultar amanhã.</i>"
...
def escutar_telegram():
    print("\nROBÔ GLOBAL DE ANÁLISE DE LIGAS ATIVO!\n")
    offset = None
    while True:
        try:
                ...
                for result in res.get("result", []):
                    ...
                    if from_chat == TELEGRAM_CHAT_ID:
                        if texto in ["/hoje", "hoje", "/jogos", "jogos", "/start"]:
                            enviar_telegram("🔎 <i>Varrendo ligas mundiais e cruzando estatísticas...</i>", from_chat)
